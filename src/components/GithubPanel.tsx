...
import { workspaceDataOnce } from "../lib/storage";
...
export function GithubPanel() {
    ...
    workspaceDataOnce(workspaceId)
      .then((workspace) => {
        ...
        try {
          ...
          if (stored) setConfig(JSON.parse(stored) as Stored);
        } catch {
          /* keep the defaults */
        }
        try {
  ...
  useEffect(() => {
    if (!ready) return;
    setSetting(workspaceId, CONFIG_KEY, JSON.stringify(config)).catch(() => {});
          ...
          <button
            onClick={() =>
              run("pull", async () => {
                ...
                // Replaces the collection: the repository is the source of
                // truth when you pull.
